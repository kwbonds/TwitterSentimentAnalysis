---
title: "Sentiment Analysis in Tweets"
author: |
        | Kevin Bonds<br>
...
The github repo for my work can be found here: https://github.com/kwbonds/TwitterSentimentAnalysis. The file is > 50 MB, so I have not included it in the repo. You will need to download it from the source above and place it in a file called _data_ in your working directory (see code below).
...
```{r grid_plot, fig.width=10, warning=FALSE, message=FALSE}
...
So, now we have `r nrow(train)` tweets. Check proportions just to be safe.
...
train_tokens <- quanteda::tokens(train$SentimentText, what = "word", 
