...
```{r}
count(raw_tweets[which(raw_tweets$text_length > 280),])
```

Looking at these we see a few more example like above, but also see a bunch or gargage text (i.e. special characters). We'll remove special characters later that will take care of this. We'll also remove incomplete cases after cleaning in case we are left with only empty strings after all of the cleaning. 
